---
name: xcode-fix
description: Diagnose and fix Xcode build errors from xcodebuild output
disable-model-invocation: true
argument-hint: "[paste error or leave blank to build first]"
allowed-tools: Bash(xcodebuild *), Bash(xcodegen *), Read, Edit, Glob, Grep
---

# Fix Xcode Build Errors

Diagnose and fix compilation errors in this iOS project.
...
| `Type 'X' has no member 'Y'` | Check API changes, use correct property/method name |
...
| `Immutable value 'X' may only be initialized once` | Use `var` instead of `let` |
| `'X' is deprecated` | Replace with the suggested alternative API |
| `Ambiguous use of 'X'` | Add explicit type annotation to disambiguate |

5. **Rebuild** after applying fixes to verify they work.

6. **Report** what was fixed and if any issues remain.

## Important

- Never suppress warnings with `@available` unless the deprecated API has no replacement for the deployment target
