...
CN 版 iOS 工程可以构建，阿里云 ECS 上的后端进程在线，正式域名 DNS 与 HTTPS 已完成，iOS 配置已切到阿里云域名，本地最新后端代码也已部署到阿里云并完成服务器端构建，生产 App Key 强制校验已启用，法务网页已部署在同一域名 `/legal/` 下。产品已从订阅/广告模型调整为 StoreKit 消耗型成片制作包：首发优惠价 ¥3.8/张，常规目标价 ¥9.9/张，每包 3 次生成机会。App Store Connect 已创建消耗型商品并保存中国大陆首发价格，但当前仍不能推荐上线，主要阻断点是真机端到端回归尚未完成、ICP/App 备案信息需要确认、内购审核截图缺失，以及成本模型需要重算。
